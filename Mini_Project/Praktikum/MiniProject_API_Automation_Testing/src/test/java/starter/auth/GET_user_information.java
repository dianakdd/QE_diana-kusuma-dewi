...
public class GET_user_information {
    ...
    public void validateValidDataForUserInformation(){
        restAssuredThat(response->response.body("data.Fullname",equalTo("Diana")));
        restAssuredThat(response->response.body("data.Email",equalTo("dianakd@gmail.com")));
        restAssuredThat(response->response.body("data.Password",equalTo("dianacantik")));
    }
}
